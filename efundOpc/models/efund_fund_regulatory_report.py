...
class EfundFundRegulatoryReport(models.Model):
    ...
    name = fields.Char(
        string="Référence",
        required=True,
        default=lambda self: _('État réglementaire OPCVM')
    )
    ...
    snapshot_id = fields.Many2one(
        ...
        string="Composition réelle",
        required=True,
        ondelete='restrict'
    )

    control_id = fields.Many2one(
        'efund.fund.allocation.control',
        string="Contrôle de conformité",
        required=True,
        ondelete='restrict'
    )
    ...
    conclusion = fields.Text(
        string="Conclusion réglementaire",
        readonly=True
    )
