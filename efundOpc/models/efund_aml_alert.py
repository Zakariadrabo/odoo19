...
class FundAmlAlert(models.Model):
    ...
    alert_date = fields.Date(string="Date alerte",default=fields.Date.context_today)
    trigger = fields.Text(string="Déclencheur",help="Reason why this alert was generated (e.g., unusual transaction pattern, high-risk jurisdiction, etc.)")
    severity = fields.Selection([('info', 'Info'),('suspicious', 'Suspicious'),('critical', 'Critical')
    ], string="Sévérité", default='suspicious', required=True, tracking=True)
    ...
    score = fields.Integer(string="Risque Score", help="Numeric score representing the AML risk level.")
    ...
    assigned_to = fields.Many2one('res.users', string="Attribué à", tracking=True)
    resolution_notes = fields.Text(string="Resolution Notes")
    document_ids = fields.Many2many('ir.attachment',string="Documents attaché",help="Attach relevant documents or investigation reports")
    closed_at = fields.Datetime(string="Date clôture", readonly=True)
